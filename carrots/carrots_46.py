...
x=SPRITE(19.4,1,19.9,1,45,0,46)
##make bins
data=[]
for low,high in zip([0]+range(1,47,5),[0]+range(5,52,5)):
    count=0
    print low,high
    for i in x[1]:
        if low<=i<=high:
...
ax.bar(range(11),data,align='center',width=.75,color='k')
...
ax.set_xticklabels(['0']+[str(i)+'-'+str(j) for i,j in zip(range(1,42,5),range(5,46,5))]+['46'])
